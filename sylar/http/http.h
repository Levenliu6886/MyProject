...
//HTTP方法枚举
enum class HttpMethod {//enum class:枚举类 因为用枚举的话，它的范围是全局域的，容易和下面枚举命名一样导致冲突，枚举类就没有冲突了
...
public:
    ...
    void delHead(const std::string& key);
...
private:
    ...
    HttpMethod m_method;
    uint8_t m_version;      //0x10:1.0   0x11:1.1    0x20:2.0
    bool m_close;           //是否自动关闭

    std::string m_path;     //url相关部分 请求路径
...
public:
    ...
    const MapType& getHeads() const { return m_headers; }
...
private:
    ...
    bool m_close;
    std::string m_body;
    std::string m_reason;   //响应码后面的一串东西，响应原因
    MapType m_headers;
};
