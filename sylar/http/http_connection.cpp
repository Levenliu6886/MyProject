...
HttpResponse::ptr HttpConnection::recvResponse() {
    ...
    do {
        ...
        if(len <= 0) {
            close();
            //SYLAR_LOG_INFO(g_logger)<<"lalala";
            return nullptr;
        }
        len += offset;
        data[len] = '\0';
    ...
    if(client_parser.chunked) {
        ...
        do {
            ...
            //SYLAR_LOG_INFO(g_logger) << "content len=" << client_parser.content_len;
        ...
        if(length > 0) {
            ...
            if(length >= offset) {
                ...
                //append是将string后面加上data开始的offset个数据
...
HttpResult::ptr HttpConnection::DoGet(const std::string& url
    ...
    if(!uri) {
        //make_shared在动态内存中分配一个对象并初始化它，返回指向此对象的shared_ptr,和new很像
...
void HttpConnectionPool::ReleasePtr(HttpConnection* ptr, HttpConnectionPool* pool) {
    ++ptr->m_request;
    if(!ptr->isConnected() || (ptr->m_createTime + pool->m_maxAliveTime) >= sylar::GetCurrentMS()
...
HttpResult::ptr HttpConnectionPool::doRequest(HttpRequest::ptr req
    ...
    if(!sock) {
        return std::make_shared<HttpResult>((int)HttpResult::Error::POOL_INVALID_CONNECTION, nullptr,
                                    "pool host: " + m_host + " port:" + std::to_string(m_port) + " pool invalid connection");
    }
    sock->setRecvTimeout(timeout_ms);
    int rt = conn->sendRequest(req);
    if(!rt) {
    ...
    if(!rsp) {
        return std::make_shared<HttpResult>((int)HttpResult::Error::TIMEOUT, nullptr,
                                    "recv response timeout: " + sock->getRemoteAddress()->toString() + " timeout_ms: "
                                    + std::to_string(timeout_ms));
    }
    return std::make_shared<HttpResult>((int)HttpResult::Error::OK, rsp, "ok");
}
