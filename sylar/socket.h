...
public:
    ...
    int recvFrom(void* buffer, size_t length, const Address::ptr from, int flags = 0);//udp
    int recvFrom(const iovec* buffers, size_t length, const Address::ptr from, int flags = 0);
    ...
    int gerProtocol() const { return m_protocol; }

    bool isConnected() const { return m_isConnected; }
    bool isValid() const;
    int gerError();

    //输出信息到流中,把sock的相关信息都输出来
