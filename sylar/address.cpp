...
template<class T>
static T CreateMask(uint32_t bits) {//创建掩码
...
bool Address::Lookup(std::vector<Address::ptr>& result, const std::string& host,
    ...
    hints.ai_socktype = type; //1流、2数据报
    ...
    if(!host.empty() && host[0] == '[') {
        //memchr可以寻找一段字符串中是否有我们所需要的某个字符, 如果找到字符，则返回的是一个指针，这个指针指向字符数组中该字符
...
bool Address::GetInterfaceAddress(std::multimap<std::string, std::pair<Address::ptr, uint32_t>>& result,
    ...
    //getifaddrs获取本地网络接口信息，将之存储于链表中，链表头结点指针存储于__ifap中带回，函数执行成功返回0，失败返回-1，且为errno赋值。
...
bool Address::GetInterfaceAddress(std::vector<std::pair<Address::ptr, uint32_t>>& result,
    ...
    //二分查找iface， 返回的是两个迭代器
    //equal_range()返回std::pair对象，其first和second成员都成为迭代器，
    //且分别指向输入序列中所有值等于 val 的元素所组成的子序列的起始及末尾（即最后一个元素之后的位置）位置。
...
IPAddress::ptr IPAddress::Create(const char* address, uint16_t port) {
    ...
    //ai_flagsAI_NUMERICHOST当此标志置位时，此标志表示调用中的节点名必须是一个数字地址字符串
    ...
    try {
        //动态转换是因为Address::Create方法返回的是address智能指针，所以需要把它转成ipaddress指针，如果是unknown那种类型就会被转失败，返回null
...
IPv4Address::IPv4Address(uint32_t address, uint16_t port) {
    ...
    m_addr.sin_port = byteswapOnLittleEndian(port);//转成网络字节序(大端)
...
UnixAddress::UnixAddress() {
    ...
    //offsetof函数，能自动返回结构对象中，某变量距离结构体对象首地址的偏移值, 第一个参数是一个结构体，第二个参数是这个结构体中变量的名字
