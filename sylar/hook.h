...
//当它与"C"一起连用时，如：extern “C” void func(int a)；
//则告诉编译器在编译 func 函数名时按着 C 的规则去翻译相应的函数名而不是 C++ 的
extern "C" {

//sleep
//定义函数指针类型，必须使用typedef，方法就是在“定义函数指针变量”加上typedef
typedef unsigned int (*sleep_fun)(unsigned int seconds);//定义了一个类型sleep_fun
//extern int g_Int; 它的作用就是声明函数或全局变量的作用范围的关键字，
//其声明的函数和变量可以在本模块或者其他模块中使用
extern sleep_fun sleep_f;

typedef int (*usleep_fun)(useconds_t usec);
extern usleep_fun usleep_f;

typedef int (*nanosleep_fun)(const struct timespec *req, struct timespec *rem);// 创建一个函数指针类型
extern nanosleep_fun nanosleep_f;//声明它的一个成员变量，

//socket
typedef int (*socket_fun)(int domain, int tyoe, int protocol);
