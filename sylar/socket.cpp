...
Socket::ptr Socket::accept() {
    ...
    int newsock = ::accept(m_sock, nullptr, nullptr);//两个冒号代表这个函数不是自己的函数是全局的
    if(newsock == -1) {
...
int Socket::recvFrom(const iovec* buffers, size_t length, const Address::ptr from, int flags) {
    if(isConnected()) {
        msghdr msg;
        memset(&msg, 0, sizeof(msg));
        msg.msg_iov = (iovec*)buffers;
...
void Socket::newSock() {
    ...
    if(SYLAR_LICKLY(m_sock != -1)) {
        ...
        SYLAR_LOG_ERROR(g_logger) << "sock(" << m_family
